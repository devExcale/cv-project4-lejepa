...
def get_dataloaders(
	...
	if paradigm not in ("std", "lejepa"):
		raise ValueError("paradigm must be either 'std' or 'lejepa'")

	train_transform, val_transform = get_transforms(dataset_name)
	spec = DATASETS[dataset_name]
	full_train = load_dataset(spec["hf_path"], split="train", cache_dir=data_dir)
	...
	if paradigm == "lejepa":
		...
		try:
			splits = full_train.train_test_split(
				...
				seed=seed,
				stratify_by_column=spec["label_key"],
			)
		except (ValueError, TypeError):
			splits = full_train.train_test_split(test_size=val_fraction, seed=seed)
