...
def train_supervised(
	...
	checkpointer = MilestoneCheckpointer(dataset=dataset_name, arch=arch, paradigm=paradigm)
	history = {"train_loss": [], "train_acc": [], "val_loss": [], "val_acc": []}
	start_epoch = 1

	if resume:
		start_epoch, _, history = checkpointer.load_best(model, optimizer, scheduler, history, device)
	...
	for epoch in range(start_epoch, epochs + 1):
		...
		current_lr = optimizer.param_groups[0]["lr"] if is_vit else scheduler.get_last_lr()[0]
		print(
		...
		# Evaluates both overall best and closest milestone
		checkpointer.step(
			epoch=epoch,
			val_acc=val_acc,
			model=model,
	...
	print(
		...
		f" Best Val Acc: {checkpointer.best_acc:.2f}%\n"
	)
